...
export function getFix(opts={}){
  return new Promise((res,rej)=>{
    ...
    navigator.geolocation.getCurrentPosition(p=>res(p), e=>rej(e), {enableHighAccuracy:true, timeout:10000, maximumAge:0, ...opts});
  });
}
export function startWatch(onFix,onError,opts={}){
  if(!navigator.geolocation){ onError?.(new Error("GPS not supported")); return null; }
  return navigator.geolocation.watchPosition(p=>onFix(p), e=>onError?.(e), {enableHighAccuracy:true, maximumAge:1000, timeout:10000, ...opts});
}
